...
const RegisterForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await axiosInstance.post('/api/auth/register', {username, password, email});
      ...
      <div onSubmit={handleSubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        ...
        <div className="text-center mt-4">
          <p className="cursor-pointer text-sm text-gray-600">
            Already have an account? <span onClick={()=>window.location.href="/login"} className="text-blue-500 hover:text-blue-700">Sign In</span>
          </p>
        </div>
      </div>
    </div>
